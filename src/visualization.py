...
def learning_curve_cavity(acc_val_list, acc_train_list, loss_train_list):
    ...
    fig.savefig(
        f"{os.path.dirname(sys.path[0])}/output/cavity_models/cavity_learning_curve.pdf",
        bbox_inches="tight",
    )
    plt.close()
...
def learning_curve_ds_with_errorbars():
    ...
    model_files = glob.glob(f"{os.path.dirname(sys.path[0])}/output/ds_models/**/*.pkl")
    ...
    ax2.legend(lns, labs, loc="center right")
    fig.savefig(
        f"{os.path.dirname(sys.path[0])}/output/ds_models/ds_learning_curve_with_errorbars.pdf",
        bbox_inches="tight",
    )
    plt.close()
...
def hist_plot(df, dataset_key):
    ...
    fig.savefig(
        f"{os.path.dirname(sys.path[0])}/output/{dataset_key}/total_pred_hist.pdf",
        bbox_inches="tight",
    )
    plt.close()
    ...
    for pdbid in prot_names:
        ...
        fig.savefig(
            f"{os.path.dirname(sys.path[0])}/output/{dataset_key}/{pdbid}_pred_hist.pdf",
            bbox_inches="tight",
        )
        plt.close()


def hist_plot_all(df, dataset_key, p):
    """
    Plots the histogram of ddGs from a dataframe.
    ...
    # Plot all data
    print(f"Plotting histogram for data set: {dataset_key}")
    if dataset_key == "GnomAD_ClinVar":
        x = df["RaSP;score_ml"].to_numpy()
        print(f"Number of PDBs: {len(np.unique(df['pdb'].to_numpy()))}")
        print(f"Number of PDB chains: {len(np.unique(df['pdb_chain'].to_numpy()))}")
        print(f"Number of ddGs: {len(x)}")
    else:
        x = df["score_ml"].to_numpy()
        print(f"Number of PDBs: {len(np.unique(df['pdbid'].to_numpy()))}")
        print(f"Number of ddGs: {len(x)}")
    fig, ax = plt.subplots()
    ax.hist(x, color="blue", bins=100, rwidth=1.0, weights=[1 / 10 ** p] * len(x))
    ax.grid(False)
    ax.set_xlabel("RaSP \u0394\u0394G [kcal/mol]")
    ax.set_ylabel(f"Variant count x $10^{str(p)}$")
...
def make_scatter(x, y, x_vs_y, dataset_key, pdb):
    ...
    if len(x) > 2 and len(y) > 2:
        ...
        if len(x) > 1000:
            ...
            fig.colorbar(
                density, label="Number of points per pixel", fraction=0.046, pad=0.04
            )
            ax.set_aspect("equal")
        else:
        ...
        if x_vs_y == "exp_vs_ml":
            ...
            x_vs_y == "rosetta_vs_ml"
            or x_vs_y == "rosetta_vs_ml_buried"
            or x_vs_y == "rosetta_vs_ml_exposed"
        ):
        ...
        if (
            dataset_key == "ProTherm_with_Protein_G"
            or dataset_key == "S669"
            or dataset_key == "Ssym_dir"
            or dataset_key == "Ssym_inv"
            or dataset_key == "VAMP"
        ):
            ...
            for item in (
                ...
                + [
                    x.txt._text
                    for x in ax.artists
                    if type(x) == matplotlib.offsetbox.AnchoredText
                ]
            ):
        ...
        if pdb == None:
            ...
            plot_name = pdb
        fig.savefig(
            f"{os.path.dirname(sys.path[0])}/output/{dataset_key}/{plot_name}_{x_vs_y}_scatter.pdf",
            bbox_inches="tight",
        )
        plt.close()
...
def make_scatter_combined(df, dataset_key, x_vs_y, prot_names):
    ...
    if dataset_key == "ProTherm_with_Protein_G":
        ...
        for i, pdbid in enumerate(prot_names):
            ...
            ax.scatter(
                ...
                s=2,
                c=colors[i],
                label=f"{prot_names[i]}, $\\rho$: {pearson_r:.2f}, MAE: {mae:.2f} kcal/mol, ME: {me:.2f} kcal/mol",
            )

            # Set xy limits and labels
            xylim = ((-2, 7), (-2, 7))
            ax.set_xlim(xylim[0][0], xylim[0][1])
            ax.set_ylim(xylim[1][0], xylim[1][1])
            ax.set_xticks([-1, 1, 3, 5, 7])
        ...
        plot_name = "combined"
        fig.savefig(
            f"{os.path.dirname(sys.path[0])}/output/{dataset_key}/{dataset_key}_{plot_name}_{x_vs_y}_scatter.pdf",
            bbox_inches="tight",
        )
        plt.close()
    ...
    if dataset_key == "VAMP":
        ...
        for i, pdbid in enumerate(prot_names):
            ...
            plot_name = "combined"
            fig.savefig(
                f"{os.path.dirname(sys.path[0])}/output/{dataset_key}/{dataset_key}_{plot_name}_{x_vs_y}_scatter_{pdbid}.pdf",
                bbox_inches="tight",
            )
            plt.close()
...
def scatter_plots(df, dataset_key):
    ...
    # Create plot: ML vs Rosetta - Exposed vs. Buried
    if dataset_key == "Rosetta":
        # Extract SASA
        df["sasa"] = df["resenv"].map(lambda elem: elem.sasa)

        # Exposed
        df_high = df[df["sasa"] > 0.2]
        x = df_high["score_rosetta"]
        y = df_high["score_ml"]
        y = y[(x >= -1) & (x <= 7)]
        x = x[(x >= -1) & (x <= 7)]
        make_scatter(x, y, "rosetta_vs_ml_exposed", dataset_key, None)

        # Buried
        df_low = df[df["sasa"] <= 0.2]
        x = df_low["score_rosetta"]
        y = df_low["score_ml"]
        y = y[(x >= -1) & (x <= 7)]
        x = x[(x >= -1) & (x <= 7)]
        make_scatter(x, y, "rosetta_vs_ml_buried", dataset_key, None)
    ...
    if dataset_key == "ProTherm_with_Protein_G":
        # Create plot: ML/Rosetta vs Exp - All --> Individual colors
        prot_names = df["pdbid"].unique()
...
def make_heatmap(dataset_key, mat, aa_list, description):
    ...
    fig.savefig(
        f"{os.path.dirname(sys.path[0])}/output/{dataset_key}/heatmap_{description}.pdf",
        bbox_inches="tight",
    )
    plt.close()
...
def homology_plot(df, dataset_key):
    ...
    for i, pdb in enumerate(pdb_list):
        ...
        anchored_text = AnchoredText(
            name_dict[pdb], loc="upper right", prop={"size": 12}
        )
        ax.add_artist(anchored_text)
        ax.set_xticks([92.5, 77.5, 62.5, 47.5, 32.5])
        ax.set_yticks([1.00, 0.75, 0.5, 0.25])
        ax.set_yticklabels(["1.00", "0.75", "0.50", "0.25"], fontsize="12")
        lns = l1 + l2
    ...
    ax.invert_xaxis()
    ax.set_xticklabels(["bin1", "bin2", "bin3", "bin4", "bin5"], fontsize="14")
    ax.text(vline_list[0] + 2.5, 0.05, "100%", color="dimgrey", fontsize="14")
    ax.text(vline_list[1] + 2.5, 0.05, "85%", color="dimgrey", fontsize="14")
    ax.text(vline_list[2] + 2.5, 0.05, "70%", color="dimgrey", fontsize="14")
    ax.text(vline_list[3] + 2.5, 0.05, "55%", color="dimgrey", fontsize="14")
    ax.text(vline_list[4] + 2.5, 0.05, "40%", color="dimgrey", fontsize="14")
    ax.text(vline_list[5] + 2.5, 0.05, "25%", color="dimgrey", fontsize="14")
    fig.legend(lns, labels, loc="upper center", ncol=2, prop={"size": 18})
    fig.savefig(
        f"{os.path.dirname(sys.path[0])}/output/{dataset_key}/homology_plot.pdf",
        bbox_inches="tight",
    )
...
def plot_gnomad_clinvar(xlim=[-1, 7]):
    ...
    # Plot histogram of RaSP predictions
    hist_plot_all(df, "GnomAD_ClinVar", 4)
    ...
    for ax in axs:
        ax.set_xlim(xlim)
        ax.set_xlabel("RaSP \u0394\u0394G [kcal/mol]", fontsize=18, labelpad=15)
        ax.set(ylabel=None)
        ax.tick_params(labelsize=18)

    fig.align_labels()
    fig.savefig(
        f"{os.path.dirname(sys.path[0])}/output/Human/rasp_gnomad_clinvar_{xlim[0]}_to_{xlim[1]}.pdf",
        bbox_inches="tight",
    )
