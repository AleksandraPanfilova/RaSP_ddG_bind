...
def main():
    ...
    pdb_dir = f"{os.path.dirname(sys.path[0])}/data/test/ddG_bind/structure/"
    ...
    pdb_filenames_cleaned = sorted(
            glob.glob(
                f"{os.path.dirname(sys.path[0])}/data/test/ddG_bind/structure/cleaned/*.pdb"
            )
        )
    ...
    df.to_csv("../data/test/ddG_bind/ddG_Rosetta/ddg.csv", index=False)

    # Load Rosetta ddGs
    df_rosetta = pd.read_csv(
                        f"{os.path.dirname(sys.path[0])}/data/test/ddG_bind/ddG_Rosetta/ddg.csv"
                                )
    
    # Create temporary residue environment datasets to more easily match ddG data
    pdb_filenames_parsed = sorted(glob.glob(f"{os.path.dirname(sys.path[0])}/data/test/ddG_bind/structure/parsed/*coord*"))
    dataset_key = "ddG_bind"
